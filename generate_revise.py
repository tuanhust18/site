...
def generate_html(publications):
    ...
    for idx, pub in enumerate(publications):
        ...
        # Nếu có DOI thì click mở DOI, nếu không thì không làm gì khi click
        link_to_open = f'https://doi.org/{doi}' if doi else '#'

        list_items += f"""
        <li class="list-group-item mb-3" style="cursor:pointer;" onclick="if('{doi}') window.open('{link_to_open}', '_blank')">
...
<head>
    ...
    <style>
        body {{
            ...
            background-color: #1e2328;
            color: white;
            font-size: 16px; /* Tăng font size toàn trang lên 16px */
        }}
        h2 {{
            margin-bottom: 2rem;
            color: white;
            font-size: 24px; /* Tiêu đề trang lớn hơn */
        }}
        .list-group-item {{
            background-color: transparent;
            border: none;
        ...
        .publication-title {{
            font-size: 18px; /* Tăng font size tiêu đề bài báo lên 18px */
            margin-bottom: 0.25rem;
        }}
        .text-muted {{
            color: #ccc !important;
